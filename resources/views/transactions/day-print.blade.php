...
<head>
    <meta charset="UTF-8">
    <title>{{optional($invoices->first())->customer_name}} Day Vouchers</title>
    <style>
        @media print {
            .page-break {
                display: block;
...
<body translate="no" onload="javascript:window.print()">
    ...
    <div id="invoice-POS" @if($invoiceIndex > 0) class="page-break" @endif>
        <center id="top">
             <div class="info">
        ...
        <div id="bot">
            <table class="bot-1-table">
                <tr>
                    @php
                    $parts = explode('-', $invoice->crv_no ?? $invoice->cpv_no );
                    $invoice_first_part = $parts[1] ?? ($invoice->crv_no ?? $invoice->cpv_no);
                    @endphp
            ...
            <table class="bot-5-table">
                ...
                @endphp
                <tr>
                    <td class="payable-heading">Previous {{$bal >= 0 ? 'Receivable' : 'Payable' }} :</td>
