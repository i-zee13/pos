...
$(document).on('input', '.qty-input', function () {
    ...
    var current_product_qty = 0;
    var product_amount = $(this).attr('data-value');
    var current_product_price = 0;
    // Immutable row's initial qty (invoice qty on load/render)
    var row_original_qty = parseInt($(this).attr('data-quantity') || 0);
    $('.amount_received').val($('.paid_amount').text());
    setTimeout(function () {
      $('.amount_received').trigger('input');
    }, 500);
    ...
    sales_product_array.filter(function (data) {
      if (data.product_id == current_product_id) {
        ...
        // Cache original qty once. Do NOT use data.qty (it changes on every input event).
        if (data.original_qty === undefined || data.original_qty === null || data.original_qty === '') {
          data.original_qty = row_original_qty || parseInt(data.qty || 0);
        }
        var original_qty = parseInt(data.original_qty || 0);

        var available_stock = parseInt(data.stock_in_hand || 0);
        // In edit screen, stock_in_hand is remaining stock excluding this invoice line.
        // Allow user to increase up to remaining + original invoice qty.
        if (data.sale_prod_id && data.sale_prod_id !== '') {
          available_stock = available_stock + original_qty;
        }
        current_product_qty = available_stock;
