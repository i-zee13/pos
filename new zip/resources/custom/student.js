...
if($('#account_type').val()==1){
    $('.relation_type').css('display','none');
}
    ...
    $("#save").on('click', function () {
        ...
        if (email != '' && !emailReg.test(email)) {
         ...
         if (primary_cellphone != '' && (!numberReg.test(secondary_phone) || !numberReg.test(primary_cellphone))) {
            $('#notifDiv').fadeIn();
            $('#notifDiv').css('background', 'red');
...
function edit_client(client_id) {
    ...
    $.ajax({
           url: `/edit/${client_id}`,
           success: function (response) {
            window.location = "/student-edit/" + id;
        }
    });
    ...
    if (!not_found) {
        ...
        all_cust[current_cust_active_page].map(function (element) {
            ...
            <div class="col colStyle" style="max-width:180px">
                ...
                <a style="color: white" href="/student-edit/${element['id']}" class="btn cusDetail">Edit</a>
            </div>
        </div>
     </div>`);
        ...
        <div class="_product-card ${element['is_active'] == '0' ? '_deactive-cus' : ''}">
         <img src="${element['profile_img'] ? '/storage/' + element['profile_img'] : '/images/avatar.svg'}" alt="" >
            ...
            <div class="con_info pt-0 PB-20">
                ...
                <div class="PT-20">
                    ...
                    <a style="color: white"href="/student-edit/${element['id']}" class="btn cusDetail-th ">Edit</a>
                </div>
...
$(document).on('change', '#account_type', function () {
    if ($(this).val()== 2) {
        $('.relation_type').css('display','block');
    }else{
        $('.relation_type').css('display','none');
    }
});
