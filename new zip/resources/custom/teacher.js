...
$(document).ready(function () {
    ...
    $("#save").on('click', function () {
        ...
        let dirty2 = false;
        $('.student_required').each(function () {
        ...
        if ($('select[name="acquisition_source"]').val() == 0 || $('select[name="gender_id"]').val() == 0 || $('select[name="country_id"]').val() == 0 || $('select[name="state_id"]').val() == 0 || $('select[name="city_id"]').val() == 0) {
            $('#notifDiv').fadeIn();
            $('#notifDiv').css('background', 'red');
            $('#notifDiv').text('Please provide all the required information (*)');
            setTimeout(() => {
        ...
        if (email != '' && !emailReg.test(email)) {
         ...
         if (primary_cellphone != '' && (!numberReg.test(secondary_phone) || !numberReg.test(primary_cellphone) || !numberReg.test(whatsapp_no))) {
            $('#notifDiv').fadeIn();
            $('#notifDiv').css('background', 'red');
            $('#notifDiv').text('Please Enter Right Contact Numbers (*)');
            setTimeout(() => {
                $('#notifDiv').fadeOut();
            }, 3000);
            return;
...
function edit_client(client_id) {
    ...
    $.ajax({
           url: `/edit/${client_id}`,
           success: function (response) {
            window.location = "/teacher-edit/" + id;
        }
    });
            ...
            response.client_document.forEach((element, key) => {
                $('#documentTable tbody').append(`<tr>
                        ...
                        <td>
                            ...
                            <img class="doc-img"  src="/storage/${element['doc_front_image']}"  height="35px"> </a>
                            </td>
                        <td>
                            ...
                            <img class="doc-img"  src="/storage/${element['doc_back_image']}" height="35px"></a>
                            </td>
...
$(document).on('click', '.customer_page_link', function () {
    ...
    if (!not_found) {
        ...
        all_cust[current_cust_active_page].map(function (element) {
            ...
            <div class="col colStyle" style="max-width:355px">
                <div style="display:table; margin-top: -5px; margin-bottom:-5px;">
                    <div class="_emp-D"><img class="doc-img" 
                            src="${element['profile_img'] ? '/storage/' + element['profile_img'] : '/images/avatar.svg'}"
                            alt=""></div>
            ...
            <div class="pt-5">${element['status'] == 1 ? 'Active' : element['status'] == 2 ? 'In Active' : element['status'] == 3 ? 'Churnned' : element['status'] == 4 ? 'Blocked': element['status'] == 5 ? 'Deleted' : 'NA'}</div>

            </div>
            <div class="col colStyle" style="max-width:180px">
                <a  href="/client-view/${element['id']}" class="btn cusDetail">Detail</a>
                <a style="color: white" href="/teacher-edit/${element['id']}" class="btn cusDetail">Edit</a>
            </div>
        </div>
     </div>`);
        ...
        <div class="_product-card ${element['is_active'] == '0' ? '_deactive-cus' : ''}">
         <img class="doc-img"  src="${element['profile_img'] ? '/storage/' + element['profile_img'] : '/images/avatar.svg'}" alt="" >
            ...
            <div class="con_info pt-0 PB-20">
                ...
                <div class="PT-20">
                    ...
                    <a style="color: white"href="/teacher-edit/${element['id']}" class="btn cusDetail-th ">Edit</a>
                </div>
...
function openModel(id) {
    ...
    $('.preview  .modal-body').html('<img class="doc-img"  src="/storage/' + `${id}` + '" class="cnicCardimg" />');
};
    ...
    $('#upload_excel_form').ajaxSubmit({
        ...
        success: function (response) {
            ...
            if (response.status == 'failed') {
                ...
                $('#notifDiv').fadeIn();
                $('#notifDiv').css('background', 'red');
                $('#notifDiv').text('Failed to add Students at the moment');
                setTimeout(() => {
                    $('#notifDiv').fadeOut();
                }, 3000);
                if (response.uploading_error.length > 0) {
                    ...
                    response.uploading_error.forEach(element => {
                        $('#not_uploadable_clients tbody').append(`
                                    <tr>
                                        <td>${element.count + 1}</td>
                                        <td>${element.email ? element.email : "NA"}</td>
                                        <td>${element.instructor_code ? element.instructor_code : "NA"}</td>
            ...
            //                         <td>${element.count + 1}</td>
            //                         <td>${element.email ? element.email : "NA"}</td>
            //                         <td>${element.user_name ? element.user_name : "NA"}</td>
            //                         <td>${element.student_code ? element.student_code : "NA"}</td>
            //                         <td>${element.reason}</td>
            //                     </tr> `)
            //     })
