...
$(document).ready(function () {
    ...
    $(document).on('click', '.openDataSidebarForUpdateCustomerLedger', function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                response.transactions.forEach(data => {
                    ...
                    cr_sum      += data.cr;
                    dr_sum      += data.dr;
                    if (action == 'customer-ledger-jama' && data.cr > 0) {
                        console.log(data.cr)
                    ...
                    if (action == 'customer-ledger-banam' && data.dr > 0) {
                        $('#transactionTable tbody').append(`
                        <tr id='tr-${data.id}'>
                            <td>${action == operation + '-ledger-jama' ? data.crv_no : data.cpv_no}</td> 
                            <td>${action == operation + '-ledger-jama' ? data.cr : data.dr}</td>
        ...
        // $('.amount').val(response.last_inserted.)
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            response.customers.forEach((element, key) => {
                ...
                if (action === 'customer-ledger-banam' && element.cpv_no) {
                ...
                if (action === 'customer-ledger-jama' && element.crv_no) {
                    $('.subCatsListTable tbody').append(`
                            <tr>
                            <td> ${voucher}</td> 
                            <td> ${element['customer_name']}</td>
...
$(document).on('click', '.conf_irmbtn', function () {
    closeSidebar();
});
let total_amount = 0;
