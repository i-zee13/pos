...
(() => { // webpackBootstrap
    ...
    $(document).ready(function () {
            ...
            $.ajax({
                ...
                success: function success(response) {
                    ...
                    $.ajax({
                        url: '/get-countries',
                        success: function success(response) {
                            $.when(response.result.countries.forEach(function (data) {
                                $("#countries_2").append("<option data-id=\"".concat(data.id, "\" value=\"").concat(data.id, "\">").concat(data.name, "</option>"));
                            })).then(function () {
                                if (location_country) {
        ...
        function getLocation() {
            ...
            $.ajax({
                ...
                success: function success(response) {
                    ...
                    response.location.forEach(function (element, key) {
                        $('.location_table tbody').append("\n                            <tr>\n                                <td> ".concat(key + 1, " </td>\n                                <td> ").concat(element.location_name, "</td>\n                                <td> ").concat(element.phone_no, "</td>\n                                <td> ").concat(element.email, "</td>\n                                <td> ").concat(element.latitude, "</td>\n                                <td> ").concat(element.longitude, "</td>\n                                <td><a id=\"").concat(element['id'], "\"  class=\"btn btn-default btn-line m-b openDataSidebarForUpdateLocation\">Edit</a>\n                                  <a href=\"#\"   id=\"").concat(element['id'], "\" class=\"btn btn-default red-bg location_delete\"  >Delete</a>\n                                </td>\n                            </tr> "));
                    });
        ...
        function geographical_data() {
            $.ajax({
                ...
                success: function success(response) {
                    ...
                    $.when(response.result.countries.forEach(function (data) {
                        $("#countries").append("<option data-id=\"".concat(data.id, "\" value=\"").concat(data.id, "\">").concat(data.name, "</option>"));
                    })).then(function () {
                        if (country) {
                    ...
                    response.result.countries.forEach(function (data) {
                        $("#countries_2").append("<option data-id=\"".concat(data.id, "\" value=\"").concat(data.id, "\">").concat(data.name, "</option>"));
                    });
                }
            });
        ...
        $(".countries").change(function () {
            ...
            $.ajax({
                url: "/get-state-against-country/".concat(country_id),
                success: function success(response) {
                    $("#states").append("<option value=\"0\">Select State</option>");
                    $.when(response.states.forEach(function (data) {
                        $("#states").append("<option value=\"".concat(data.id, "\">").concat(data.name, "</option>"));
                    })).then(function () {
                        if (state) {
        ...
        $("#states").change(function () {
            ...
            $.ajax({
                url: "/get-city-against-states/".concat(city_id),
                success: function success(response) {
                    $("#cities").append("<option value=\"0\">Select Cities</option>");
                    $('select[name="city_id"]').val('-1').trigger('change');
                    $.when(response.cities.forEach(function (data) {
                        $("#cities").append("<option value=\"".concat(data.id, "\">").concat(data.name, "</option>"));
                    })).then(function () {
                        if (city) {
        ...
        $(".countries_2").change(function () {
            ...
            $.ajax({
                ...
                success: function success(response) {
                    $("#states_2").append("<option value=\"0\">Select State</option>");
                    $.when(response.states.forEach(function (data) {
                        $("#states_2").append("<option value=\"".concat(data.id, "\">").concat(data.name, "</option>"));
                    })).then(function () {
                        if (location_state) {
        ...
        $("#states_2").change(function () {
            ...
            $.ajax({
                ...
                success: function success(response) {
                    ...
                    $.when(response.cities.forEach(function (data) {
                        $("#cities_2").append("<option value=\"".concat(data.id, "\">").concat(data.name, "</option>"));
                    })).then(function () {
                        if (location_city) {
