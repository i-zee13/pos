...
// Main group for tenant-specific routes (no middleware here)
Route::group(['middleware' => ['web']], function () {
    // Authentication Routes for tenants (Manually Defined)
    ...
    // All routes that require authentication go into this nested group
    Route::group(['middleware' => ['auth']], function () {
        ...
        Route::delete('/delete-product-from-invoice',       [StockController::class, 'deleteProduct'])->name('delete-purchase-product');
        ...
        // Purchase Returns /
        Route::get('/get-customer-balance-products/{id}',           [PurchaseReturnController::class, 'getVendorBalance'])->name('get-customer-balance-products');
        ...
        Route::get('/get-customer-balance/{id}',    [SaleController::class, 'getCustomerBalance'])->name('get-customer-balance-sale');
        ...
        Route::get('/all-customer-ledger',      [LedgerDetailControlller::class, 'allCustomerLedger'])->name('all-customer-ledger');
    ...
    // Test route for tenant verification - now inside the tenant group
    Route::get('/tenant-test', function (?string $tenant_segment = null) {
        $host = request()->getHost();
        // The TenantServiceProvider automatically sets the default database connection
        // based on the host. We will use the default connection here.
        
        try {
            // Get the current default database connection name and database name
            $connectionName = DB::getDefaultConnection();
            $dbName = DB::connection($connectionName)->getDatabaseName();
            
            // Test if we can query the database using the default connection
            $testQuery = DB::connection($connectionName)->select('SELECT 1');
            
            // Determine tenant based on the host for display purposes
            $tenant = null;
            if (str_contains($host, 'localhost') || str_contains($host, '127.0.0.1')) {
                // Use the provided segment if it's not null and not 'tenant-test' itself
                // tenant_segment is implicitly passed by Laravel for {tenant} route parameter
                if (Route::current()->hasParameter('tenant')) {
            ...
            return response()->json([
                'status' => 'success',
                'tenant' => $tenant,
                'active_connection' => $connectionName,
                'database' => $dbName,
                'storage_path' => storage_path("tenants/{$tenant}"),
                'env_loaded' => file_exists(base_path("tenants/{$tenant}/.env")),
                'connection_test' => !empty($testQuery),
                'is_main_database' => ($dbName === 'pos' && is_null($tenant)) // Check if it's the main 'pos' database and no tenant was detected
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'status' => 'error',
                ...
                'host' => $host,
                'tenant_attempted' => $tenant // Show what tenant was being attempted
            ], 500);
        }
    });
